...
const rowKey = useRowKey("dictCode");
...
const getTable = () => {
  ...
  return new Promise((resolve) => {
    getDictTypeTable(props.params).then((data: any) => {
      loading.value = false;
...
const delArr = () => {
  ...
  window.$dialog.warning({
    ...
    negativeText: "取消",
    onPositiveClick: () => {
      delDictType(checkedRowKeys).then(() => {
        window.$msg.success("删除成功");
        getTable();
      });
    },
