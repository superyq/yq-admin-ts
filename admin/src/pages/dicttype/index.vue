...
import Search from "@/pages/dicttype/components/Search.vue";
import Table from "@/pages/dicttype/components/Table.vue";
import Form from "@/pages/dicttype/components/Form.vue";
...
const handleShow = (row: any = null) => {
  const isEdit = !!row;
  formId.value = isEdit ? row.dictCode : "";
  title.value = `${isEdit ? "编辑" : "新增"}字典数据`;
  show.value = true;
};
const handleSubmit = () => {
